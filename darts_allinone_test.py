...
# sYOLO_WEIGHTS = "/media/PRIV/Darts_ML/runs/detect/runs/darts_safe/train7/weights/best.pt"
...
def save_numbered_images_with_reference(img, detected_point=None, current_player=None, score=None,  diff=None, diff_thresh=None):
    ...
    cv2.imwrite(os.path.join(game_folder, img_filename), img)
    print(f"💾 {game_folder}/{img_filename}")
    
    # **VOLLSTÄNDIGER Dateiname für diff/debug (OHNE .jpg!)**
    num_part = img_filename.replace('.jpg', '')  # "img_1st_throw_001"
    
    # **2. Diff-Bilder mit GLEICHEM Namen wie img_*** IM ORGNER**
    if diff is not None:
        diff_filename = f"diff_{num_part}.jpg"  # diff_img_1st_throw_001.jpg
        cv2.imwrite(os.path.join(game_folder, diff_filename), diff)
        print(f"💾 {game_folder}/{diff_filename}")
    
    if diff_thresh is not None:
        diff_thresh_filename = f"diff_thresh_{num_part}.jpg"  # diff_thresh_img_1st_throw_001.jpg
        cv2.imwrite(os.path.join(game_folder, diff_thresh_filename), diff_thresh)
        print(f"💾 {game_folder}/{diff_thresh_filename}")
    ...
    debug_filename = f"debug_dart_{num_part}.jpg"
...
# -----------------------------
# **DEINE EXAKTE detect_dart_tip FUNKTION** (angepasst)
# -----------------------------
# def detect_dart_tip(img, refimg, H, board_center, radius):
#     # **KRITISCHE CHECKS ZUERST**
#     print("DETEKTIERE DART TIP JETZTTZZZZZZZZZZZZZZZZZZZZZZZZZZ")
#     if img is None or refimg is None or H is None:
#         print("❌ Fehlende Eingaben: img/refimg/H")
...
#     # **DEBUG: Speichere diff für Kontrolle**
#     cv2.imwrite("debug_diff.jpg", diff)
#     print("💾 debug_diff.jpg gespeichert – schau ob Dart sichtbar!")
    
#     print("🧠 YOLO auf diff...")
#     try:
#         model = YOLO(YOLO_WEIGHTS)
#         results = model(diff, device="cpu", conf=0.1, verbose=False)
#     except Exception as e:
#         print(f"❌ YOLO Fehler: {e}")
#         return None, None, diff, diff_thresh
...
#                 print(f"✅ Dart erkannt bei ({center_x}, {center_y}), Conf: {conf:.2f}")
#                 break  # Erster Treffer reicht
    
#     if best_tip is None:
#         print("❌ KEIN Dart erkannt – check debug_diff.jpg!")
#         return None, None, diff, diff_thresh

#     # **SICHERE PERSPECTIVE TRANSFORM**
#     print(f"🔄 Transformiere {best_tip}...")
#     point_array = np.array([[[float(best_tip[0]), float(best_tip[1])]]], dtype=np.float32)
#     if point_array.shape != (1, 1, 2):
#         print("❌ Point array ungültig!")
#         return None, None, diff, diff_thresh
        
#     warped = cv2.perspectiveTransform(point_array, H)
#     exact_tip = (int(warped[0, 0, 0]), int(warped[0, 0, 1]))
#     print(f"✅ Exact tip: {exact_tip}")
    
#     return exact_tip, best_tip, diff, diff_thresh
...
def update_stream():
    ...
    if stream_running and cap and cap.isOpened():
        ret, frame = cap.read()
        if ret:
            frame_small = cv2.resize(frame, (854, 480))
            frame_rgb = cv2.cvtColor(frame_small, cv2.COLOR_BGR2RGB)
            img = Image.fromarray(frame_rgb)
            imgtk = ImageTk.PhotoImage(image=img)
            video_label.imgtk = imgtk
            video_label.configure(image=imgtk)
        video_label.after(30, update_stream)
...
def start_stream():
    ...
    if init_camera():
        stream_running = True
        update_stream()
        btn_stream.config(text="Stream STOP", bg="red")
    else:
...
def compute_score_from_tip(exact_tip):
    ...
    score = SEGMENTS[idx]
    if r_tip > R_DOUBLE_OUTER : return 0 , f"Mis" 
...
def on_capture_reference():
    global refimg, refimg_filename, dart_counter_in_group, round_points, throws_label, points_detail_label, ref_group_counter
    ...
    try:
        ...
        # **ref_group_counter für Dateinamen erhöhen**
        ref_group_counter += 1
        ...
        status_label.config(text=f"✅ REFERENZ Runde {game_round_counter:03d}: {refimg_filename}", bg="lightgreen")
...
def on_analyze_one_dart():
    global refimg, H, player_scores, current_player, dart_counter_in_group, round_points, str_round_points
    ...
    try:
        ...
        if best_tip is None:
            ...
            # 🔥 SICHERE perspectiveTransform!
            point_array = np.array([[[float(best_tip[0]), float(best_tip[1])]]], dtype=np.float32)
            warped = cv2.perspectiveTransform(point_array, H)
            exact_tip = (int(warped[0, 0, 0]), int(warped[0, 0, 1]))
            points, str_points = compute_score_from_tip(exact_tip)
            print("Resultung Score: ", points, " ", str_points)
            points_color = "lightgreen"
        ...
        points_detail_label.config(text=points_text, bg=points_color)
        
        old_score = player_scores[current_player]
        new_score = max(0, old_score - points)
        player_scores[current_player] = new_score
        lbl_p1_var.set(f"Spieler 1: {player_scores[0]}")
        lbl_p2_var.set(f"Spieler 2: {player_scores[1]}")
        ...
        if dart_counter_in_group >= 3:
            ...
            lbl_current_var.set(f"Aktuell: Spieler {current_player+1}")
            ...
            status_label.config(text="🎉 Spieler fertig! REFERENZ für nächsten Spieler.", bg="lightblue")
            
            if current_player == 0:
                global game_round_counter, counter_label
                game_round_counter += 1
                counter_label.config(text=f"{game_round_counter:03d}")
                status_label.config(text="🎉 VOLLE RUNDE FERTIG! Neue Spielrunde gestartet.", bg="gold")
            
            round_points = []
            str_round_points = []
            
    except Exception as e:
        status_label.config(text=f"❌ FEHLER: {str(e)}", bg="red")
...
# -----------------------------
# GUI Setup MIT KALIBRIERUNG + Status-Anzeige
# -----------------------------
root = tk.Tk()
root.title("Darts Scoring - REFERENZ-GRUPPEN (3 Würfe)")
root.geometry("1400x850")

status_label = tk.Label(root, text="🟢 Darts-System | REFERENZ + 3 Würfe pro Gruppe", font=("Arial", 14), bg="lightgreen")
status_label.pack(pady=10)

# **STATUSZEILE: Exaktes Format**
...
# **PUNKTE exakt: "PTS: 60 => 60"**
tk.Label(status_counter_frame, text="|", font=("Arial", 12)).pack(side=tk.LEFT)
points_detail_label = tk.Label(status_counter_frame, text="PTS: 0 => 0", font=("Arial", 14, "bold"), bg="lightgreen", width=36)
points_detail_label.pack(side=tk.LEFT, padx=5)

# **NACH points_detail_label.pack() einfügen:**
...
btn_calib = tk.Button(control_frame, text="🔧 DARTSCHEIBE KALIBRIEREN", 
                     command=calibrate_dartboard, bg="purple", fg="white",
                     font=("Arial", 12, "bold"), height=2)
btn_calib.pack(fill=tk.X, pady=5)

# Statt btn_calib mit calibrate_dartboard():
btn_auto_calib = tk.Button(control_frame, text="🤖 AUTO-KALIBRIEREN", 
                          command=auto_calibrate_dartboard, 
                          bg="gold", fg="black",
                          font=("Arial", 12, "bold"), height=2)
btn_auto_calib.pack(fill=tk.X, pady=5)

btn_stream = tk.Button(control_frame, text="📹 Stream START", 
                      command=lambda: start_stream() if not stream_running else stop_stream(),
                      bg="lightgreen", font=("Arial", 14, "bold"), height=2)
btn_stream.pack(fill=tk.X, pady=5)

btn_ref = tk.Button(control_frame, text="📸 REFERENZ AUFNEHMEN (leer!)", 
                   command=on_capture_reference, bg="lightblue", 
                   font=("Arial", 12, "bold"), height=2)
btn_ref.pack(fill=tk.X, pady=5)

btn_dart = tk.Button(control_frame, text="🎯 1 Dart AUSWERTEN", 
                    command=on_analyze_one_dart, bg="orange", 
                    font=("Arial", 14, "bold"), height=2)
btn_dart.pack(fill=tk.X, pady=10)

score_frame = tk.LabelFrame(control_frame, text="Scores (501)", font=("Arial", 12))
score_frame.pack(fill=tk.X, pady=10)

lbl_p1_var = tk.StringVar(value=f"Spieler 1: {player_scores[0]}")
lbl_p2_var = tk.StringVar(value=f"Spieler 2: {player_scores[1]}")
lbl_current_var = tk.StringVar(value=f"Aktuell: Spieler {current_player+1}")
