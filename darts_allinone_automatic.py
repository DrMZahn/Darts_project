...
# 🔥 **AUTO-DETECTION FUNKTION** (Hauptlogik)
def auto_detect_dart_loop():
    global last_detection_time, auto_mode_active
    
    while auto_mode_active and cap and cap.isOpened():
        try:
            # Schneller Frame für Diff (nicht full-res)
            ret, frame = cap.read()
            if not ret: 
                time.sleep(0.1)
                continue
            ...
            # **SCHNELLE YOLO-DETECTION** (conf=0.1)
            model = YOLO(YOLO_WEIGHTS)
            results = model(diff_thresh, device="cpu", conf=0.1, verbose=False, imgsz=320)  # Klein+ schnell!
            ...
            if dart_detected and (current_time - last_detection_time) > DETECTION_COOLDOWN:
                print(f"🎯 AUTO-DETECTED DART! Triggering analysis... (Cooldown: {DETECTION_COOLDOWN}s)")
                last_detection_time = current_time
                # **AUTO-TRIGGER** der Analyse!
                root.after(0, on_analyze_one_dart)  # Tkinter mainloop safe
                
            time.sleep(0.1)  # 10 FPS für Detection
...
def toggle_auto_mode():
    ...
    if auto_mode_active:
        btn_auto_toggle.config(text="AUTO AN", bg="darkred")
        print("🔥 AUTO-DETEKT AKTIV!")
    else:
        btn_auto_toggle.config(text="AUTO AUS", bg="orange")
...
#                           cv2.circle(frame_display, (x, y), 8, color, -1)
                    ...
                    # "CALIB OK" Label oben
                    # cv2.rectangle(frame_display, (10, 10), (200, 35), (0, 255, 0), -1)
                    # cv2.putText(frame_display, "CALIB OK", (15, 30), 
...
def start_stream():
    ...
    if init_camera():
        stream_running = True
        update_stream()
        btn_stream.config(text="Stream STOP", bg="gray")
    else:
...
def on_analyze_one_dart():
    ...
    try:
        ...
        print(" on_analyze_one_dart try 2 ")
        ...
        print(" on_analyze_one_dart try 3 ")
        ...
        if best_tip is None:
            ...
            str_points = " 0"
            points_color = "red"
            print(" on_analyze_one_dart try 4a ")
            ...
            print(" on_analyze_one_dart try 4b ")
        ...
        print(" on_analyze_one_dart try 5 ")
        ...
        print(" on_analyze_one_dart try 6 ")
        ...
        print(" on_analyze_one_dart try 7 ")
        # update refimage:                
        refimg = img
...
btn_calib = tk.Button(control_frame, text="KALIBRIEREN", command=calibrate_dartboard, bg="purple", fg="white", font=("Arial", 11, "bold"), height=2)
btn_calib.pack(fill=tk.X, pady=3)

btn_auto_calib = tk.Button(control_frame, text="AUTO-KALIBRIEREN", command=auto_calibrate_dartboard, bg="lightblue", font=("Arial", 11, "bold"), height=2)
...
btn_ref = tk.Button(control_frame, text="📸 REFERENZ (leer!)", command=on_capture_reference, bg="gold", font=("Arial", 11, "bold"), height=2)
btn_ref.pack(fill=tk.X, pady=5)

btn_auto_toggle = tk.Button(control_frame, text="AUTO ON toggle", 
                    command=lambda: toggle_auto_mode(),
                    bg="orange", fg="white", font=("Arial", 14, "bold"), height=2)
btn_auto_toggle.pack(fill=tk.X, pady=5)

# **MANUAL bleibt als Fallback**
btn_dart = tk.Button(control_frame, text="DETEKTIERE MANUELL", command=on_analyze_one_dart, bg="red", font=("Arial", 11, "bold"), height=2)
btn_dart.pack(fill=tk.X, pady=3)
